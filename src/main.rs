...
fn app() -> Html {
    ...
    let onsubmit = {
        let options_state = options_state.clone();
        move |_| {
             options_state.dispatch(Action::SendState);
        }
    };
    ...
    <div class="w-full justify-center container mx-auto px-4 flex flex-col items-center gap-8">
        <div class="flex-col" style={format!("display: {}",display_options)}>
            <Options {set_input} {set_grade} {inputs} {grades} />
            <input onclick={onsubmit} class="rounded-lg border bg-slate-300 p-2 active:bg-blue-200" type="submit" value="Save" />
        </div>
